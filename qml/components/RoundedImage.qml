...
//
// Ported from omarchy-tidal. Art here comes straight from the Roon Core --
// `<image_base>/<key>?scale=fit&width=...` -- so `source` is a remote URL and
// Qt's own image cache does the caching.
Item {
  id: root
  ...
  // pixels or at 34, and a queue holds a lot of rows. Set it to roughly the
  // size the image is drawn at -- allowing for a HiDPI screen -- and Qt keeps
  // that instead. 0 means "as the file is", for anything drawn at full size.
  //
