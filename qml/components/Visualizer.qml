...
//
// The data is real: `bin/omarchy-roon-cava` streams an FFT from PipeWire's
// default sink monitor, one line of magnitudes per frame. Reading the sink
// monitor is what makes this work at all for Roon: RoonBridge plays through
// PipeWire rather than seizing the ALSA device, so the audio it sends to the DAC
// is visible on the monitor. An exclusive-mode bridge would show a flat line.
//
