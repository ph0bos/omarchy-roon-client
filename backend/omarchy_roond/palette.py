...
Ported unchanged from omarchy-tidal, because it depends on nothing but image
bytes and Roon serves those from the Core exactly as TIDAL did. It is the only
module that crossed over without modification.
...
So the companion measures it. GdkPixbuf does the decoding, which the daemon already
depends on through PyGObject for MPRIS; there is no Pillow and no numpy here and none is
...
# A second, permissive pass for sleeves that are mostly black or mostly white.
# `Slow Light` by Nocturne Atlas is the case that forced this: a near-black cover
# whose few coloured pixels never clear the strict floors, so the strict pass
# returns None and the interface falls back to the theme -- losing a colour that
# is plainly there. Below these, a cover really is greyscale.
MIN_COLOURED_SHARE_DIM = 0.012
MIN_VALUE_DIM = 0.045
MIN_SATURATION_DIM = 0.14
...
def dominant_colour(pixels) -> tuple[int, int, int] | None:
    ...
    Two passes. The strict one asks for a real share of confidently coloured
    pixels. When that finds nothing -- a near-black or near-white sleeve with a
    small amount of chroma in it -- the permissive pass looks again with the
    floors dropped, because "mostly black with a red detail" is a red sleeve to
    anyone looking at it, and returning None there throws away a colour that is
    visibly present.
    """
    return (_dominant(pixels, MIN_SATURATION, MIN_VALUE, MIN_COLOURED_SHARE)
            or _dominant(pixels, MIN_SATURATION_DIM, MIN_VALUE_DIM,
                         MIN_COLOURED_SHARE_DIM))


def _dominant(pixels, min_saturation: float, min_value: float,
              min_share: float) -> tuple[int, int, int] | None:
    ...
    if coloured / len(pixels) < min_share or not buckets:
        return None
